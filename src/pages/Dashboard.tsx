...
import { Calendar, Clock, Video, Shield, CreditCard, ArrowRight, LogOut, Settings, Home, History, User } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            ...
            // Not: 'bookings' tablosundan verileri alıyoruz
            const { data: bookings, error } = await supabase
                .from('bookings') // Senin tablo adın (app_...bookings ise onu kullanır)
                .select('*')
                .eq('user_id', user.id)
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
              ...
              <TabsContent value="upcoming">
                <Card className="border-t-4 border-t-blue-900 shadow-md">
                  <CardHeader className="flex flex-row items-center justify-between pb-2">
                    ...
                    <Badge variant="outline" className="text-blue-900">{appointments.length} Kayıt</Badge>
                  </CardHeader>
                  <CardContent>
                    {appointments.length > 0 ? (
                          ...
                          <div key={app.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 bg-white border rounded-lg hover:shadow-md transition-shadow">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                ...
                                <Badge className={app.is_trial ? "bg-green-100 text-green-700 border-0" : "bg-blue-100 text-blue-700"}>
                            ...
                            <Button className="w-full sm:w-auto bg-blue-900 hover:bg-blue-800" onClick={() => handleJoinMeeting(app.meeting_url)}>
                      ...
                      <div className="text-center py-12 text-gray-500">
                        ...
                        <Button variant="link" onClick={() => navigate('/coaches')} className="text-blue-900">Hemen Randevu Al</Button>
                      </div>
                    )}
                  </CardContent>
                </Card>
          ...
          {/* SAĞ KOLON: İSTATİSTİK (Supabase'den çekilebilir ama şimdilik görsel) */}
          <div className="space-y-6">
            <Card>
                <CardHeader><CardTitle className="text-lg">İstatistikler</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                    <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
                        <div className="flex items-center gap-2"><CreditCard className="w-4 h-4 text-gray-500"/> <span>Toplam Randevu</span></div>
