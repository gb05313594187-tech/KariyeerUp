...
export default function Register() {
  ...
  // Modal States
  const [showIntro, setShowIntro] = useState(true); // Kayıt öncesi tanıtım
  const [showSetup, setShowSetup] = useState(false); // Kayıt sonrası profil tamamlama
  ...
  const t = {
    ...
    en: {
      ...
      introSub: "Let's take a quick tour before starting your AI-powered career journey.",
      introBtn: "Let's Start!",
      setupTitle: "Complete Your Profile",
      setupSub: "This info is important for us to serve you better.",
      phone: "Phone Number",
      industry: "Expertise / Industry",
      saveBtn: "Save and Continue",
      laterBtn: "Later"
    },
    // AR ve FR dilleri için benzer eklemeler yapılabilir...
  }[language || "tr"] || {}; // Fallback ekledim
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🔥 Kayıt başarılı, login'e atmak yerine profil modalını açıyoruz
      setShowSetup(true);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-amber-50 flex items-center justify-center p-6 font-sans">
      
      {/* --- TANITIM MODALI (Kayıt Öncesi) --- */}
      {showIntro && (
      ...
      {/* --- PROFİL TAMAMLAMA MODALI (Kayıt Sonrası) --- */}
      {showSetup && (
      ...
      <Card className="relative w-full max-w-lg shadow-2xl border-0 overflow-hidden">
        ...
        <CardContent className="px-8 pb-8">
          <div className="space-y-2 mb-5">
            <Label className="text-sm font-semibold text-gray-700">{t.accountType} *</Label>
            <RadioGroup
              ...
              {accountTypes.map((item) => {
                ...
                return (
                  <div key={item.value}>
                    ...
                    >
                      {isSelected && <div className={`absolute inset-0 bg-gradient-to-br ${item.gradient}`} />}
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            <Button variant="outline" type="button" className="h-10 text-sm font-semibold border-gray-200" onClick={() => handleSocialLogin("google")}>
            ...
            <Button variant="outline" type="button" className="h-10 text-sm font-semibold border-gray-200" onClick={() => handleSocialLogin("linkedin_oidc")}>
          ...
          <div className="relative mb-5 text-center">
            <span className="px-3 bg-white text-xs text-gray-400 uppercase tracking-widest">{t.or}</span>
            <div className="absolute inset-0 flex items-center -z-10"><span className="w-full border-t border-gray-100" /></div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-gray-700 uppercase tracking-tighter">{t.fullName}</Label>
                <Input required value={formData.fullName} onChange={(e) => setFormData({ ...formData, fullName: e.target.value })} className="h-10 text-sm focus:ring-red-500 rounded-xl" placeholder="Ad Soyad" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-gray-700 uppercase tracking-tighter">{t.email}</Label>
                <Input required type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="h-10 text-sm focus:ring-red-500 rounded-xl" placeholder="ornek@email.com" />
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-gray-700 uppercase tracking-tighter">{t.password}</Label>
                <div className="relative">
                  <Input required type={showPassword ? "text" : "password"} value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} className="h-10 text-sm pr-9 rounded-xl" />
                  <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400"><Eye className="w-4 h-4" /></button>
                </div>
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-gray-700 uppercase tracking-tighter">{t.confirmPassword}</Label>
                <div className="relative">
                  <Input required type={showConfirmPassword ? "text" : "password"} value={formData.confirmPassword} onChange={(e) => setFormData({ ...formData, confirmPassword: e.target.value })} className="h-10 text-sm pr-9 rounded-xl" />
                  <button type="button" onClick={() => setShowConfirmPassword((v) => !v)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400"><Eye className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2 py-2">
              ...
              <label htmlFor="kvkk" className="text-[10px] text-gray-500 leading-tight">
                <Link to="/privacy" className="font-semibold text-red-600 hover:underline">KVKK Aydınlatma Metni</Link>'ni okudum. *
              </label>
            </div>

            <Button type="submit" disabled={isLoading || !kvkkAccepted || !formData.userType} className="w-full h-12 text-base font-black bg-gradient-to-r from-red-600 to-orange-600 rounded-2xl shadow-xl shadow-red-500/20">
