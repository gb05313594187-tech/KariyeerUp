...
const statusConfig: Record<string, { label: string; cls: string }> = {
  ...
  approved: { label: "Planlandı", cls: "bg-gradient-to-r from-emerald-50 to-green-50 text-emerald-700 border-emerald-200" },
  rejected: { label: "İptal Edildi", cls: "bg-gradient-to-r from-red-50 to-rose-50 text-red-700 border-red-200" },
...
const payStatusConfig: Record<string, { label: string; cls: string }> = {
  ...
  initiated: { label: "İşlemde", cls: "bg-blue-50 text-blue-700 border-blue-200" },
  success: { label: "Eğitim Bedeli Ödendi", cls: "bg-emerald-50 text-emerald-700 border-emerald-200" },
  completed: { label: "Eğitim Bedeli Ödendi", cls: "bg-emerald-50 text-emerald-700 border-emerald-200" },
...
export default function UserDashboard() {
  ...
  const getCoachName = (id: string) => coaches.find((c) => c.id === id)?.full_name || "Mentor";
  ...
  const nextStep = useMemo(() => {
    if (!authUser) return { title: "Giriş yap ve gelişime devam et", desc: "Kişiselleştirilmiş gelişim takibi için giriş gerekli.", ctaText: "Giriş Yap", ctaHref: "/login", icon: LogIn };
    if (completion < 60) return { title: "Gelişim profilini tamamla", desc: `Profil %${completion} tamamlandı. Uzman mentor eşleşmesi için bilgilerini güncelle.`, ctaText: "Profilimi Düzenle", ctaHref: "/user/profile", icon: BadgeCheck };
    if (sessions.length === 0) return { title: "İlk gelişim seansını planla", desc: "Mentorlarımızı incele, sana uygun uzmanı seç.", ctaText: "Mentorları İncele", ctaHref: "/coaches", icon: GraduationCap };
    if (upcomingSessions.length > 0) return { title: `${upcomingSessions.length} yaklaşan seansın var`, desc: `Sonraki Gelişim: ${formatDate(upcomingSessions[0]?.selected_date)} - ${upcomingSessions[0]?.selected_time}`, ctaText: "Seanslarımı Gör", ctaHref: "/book-session", icon: Calendar };
    return { title: "Yeni gelişim seansı planla", desc: "Kariyer hedeflerini uzmanlarla güncelle.", ctaText: "Mentorları İncele", ctaHref: "/coaches", icon: Sparkles };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white pb-20">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-orange-600/20 to-amber-500/20 border border-orange-500/30 backdrop-blur-sm mb-3">
                ...
                <span className="text-sm font-bold text-orange-300 uppercase tracking-widest">Gelişim Paneli</span>
              </div>
              <h1 className="text-3xl sm:text-4xl font-black text-white tracking-tight">
                Hoş geldin, <span className="bg-gradient-to-r from-orange-400 to-amber-400 bg-clip-text text-transparent">{displayName}</span>
              </h1>
              <p className="mt-2 text-gray-400 max-w-2xl">Mentorluk seanslarını, gelişim ödemelerini ve profilini buradan yönet.</p>
            </div>
            {subscription && (
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-amber-500/20 to-orange-500/20 border border-amber-500/30">
                ...
                <span className="text-sm font-bold text-amber-300">{subscription.subscription_type || "Premium"} Danışan</span>
              </div>
            )}
          </div>
          ...
          <div className="mt-6 max-w-xl">
            <div className="flex items-center justify-between text-xs text-gray-400">
              <span>Gelişim profili tamamlanma</span>
              <span className="font-bold text-orange-400">%{completion}</span>
            </div>
            <div className="mt-2 h-2 rounded-full bg-white/10 overflow-hidden">
              <div className="h-full bg-gradient-to-r from-orange-600 to-amber-500 transition-all rounded-full" style={{ width: `${completion}%` }} />
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {[
              ...
              { label: "Planlanan", value: approvedCount },
              { label: "Bekleyen", value: pendingCount },
              ...(totalSpent > 0 ? [{ label: "Eğitim Yatırımı", value: formatCurrency(totalSpent) }] : []),
            ].map((s) => (
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="rounded-2xl border border-orange-200 bg-white shadow-lg p-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-orange-600 uppercase tracking-wide">Yol Haritanda Sıradaki Adım</p>
              ...
              <div className="flex gap-2 flex-wrap">
                <Link to={nextStep.ctaHref}>
                  <Button className="rounded-xl bg-gradient-to-r from-orange-600 to-amber-500 text-white font-semibold hover:brightness-110 shadow-lg shadow-orange-200/50">
            ...
            <CardContent className="text-sm text-gray-700 space-y-2">
              {!authUser ? (
                <div className="space-y-3">
                  <p className="text-gray-600">Gelişim takibi için giriş yapmalısın.</p>
                  <Link to="/login"><Button className="rounded-xl bg-gradient-to-r from-orange-600 to-amber-500 text-white">Giriş Yap</Button></Link>
                </div>
              ) : (
                <>
                  <div><span className="text-gray-500">Kayıtlı Email:</span> <span className="font-medium">{authUser.email}</span></div>
                  <div><span className="text-gray-500">Üyelik Tipi:</span> <span className="font-medium capitalize">{profile?.role === 'user' ? 'Danışan' : profile?.role || 'Danışan'}</span></div>
                  {subscription && (
                  ...
                  <div className="pt-2 flex gap-2 flex-wrap">
                    <Link to="/user/profile"><Button size="sm" className="rounded-xl bg-gradient-to-r from-orange-600 to-amber-500 text-white">Profile Git</Button></Link>
          ...
          <Card className="rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-orange-200 transition-all duration-300">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                Gelişim Seansları
                {sessions.length > 0 && (
                  <span className="ml-auto text-xs px-2.5 py-1 rounded-full bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 text-orange-700 font-bold">{sessions.length}</span>
                )}
              </CardTitle>
            </CardHeader>
            <CardContent className="text-sm text-gray-700">
              {!authUser ? (
                <p className="text-gray-600">Gelişim seanslarını burada yönetebilirsin.</p>
              ) : sessions.length === 0 ? (
                <div className="space-y-3">
                  <p className="text-gray-600">Henüz planlı bir mentorluk görüşmen yok.</p>
                  <Button className="rounded-xl bg-gradient-to-r from-orange-600 to-amber-500 text-white" onClick={() => navigate("/coaches")}>
                    Mentorları İncele <ArrowRight className="h-4 w-4 ml-2" />
                  </Button>
                </div>
              ) : (
                <div className="space-y-3">
                  {upcomingSessions.length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-orange-600 uppercase tracking-wide mb-2">Yaklaşan Gelişim Seansları ({upcomingSessions.length})</p>
                  ...
                  {pastSessions.length > 0 && (
                    <div>
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Tamamlanan Seanslar ({pastSessions.length})</p>
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                {(payments.length > 0 || paymentTx.length > 0) && (
                  <span className="ml-auto text-xs px-2.5 py-1 rounded-full bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 text-orange-700 font-bold">{payments.length + paymentTx.length}</span>
                )}
              </CardTitle>
            </CardHeader>
            <CardContent className="text-sm text-gray-700">
              {!authUser ? (
                <p className="text-gray-600">Ödeme geçmişini burada takip edebilirsin.</p>
              ) : payments.length === 0 && paymentTx.length === 0 ? (
                <div className="space-y-3">
                  <p className="text-gray-600">Henüz eğitim/mentorluk ödemen bulunmuyor.</p>
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    <ShieldCheck className="h-4 w-4 text-emerald-500" />
                    Güvenli ödeme altyapısı aktif.
                  </div>
                </div>
              ) : (
                <div className="space-y-3">
                  {(payments.length > 0 ? payments : paymentTx).slice(0, 4).map((p) => (
                    <div key={p.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{formatDate(p.payment_date || p.created_at)} • Eğitim Hizmeti</p>
                      </div>
                  ...
                  {totalSpent > 0 && (
                    <div className="pt-2 border-t border-gray-100 flex items-center justify-between">
                      <span className="text-xs text-gray-500">Toplam Gelişim Yatırımı</span>
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-gray-400 italic">
                    <ShieldCheck className="h-3 w-3 text-emerald-500" /> Sadece dijital danışmanlık bedelleri yansıtılır.
                  </div>
                </div>
              )}
            </CardContent>
            ...
            <Card className="rounded-2xl border border-gray-200 shadow-sm">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-base">
                  ...
                  Dijital Hizmet Faturaları
                  <span className="ml-auto text-xs px-2.5 py-1 rounded-full bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 text-orange-700 font-bold">{invoices.length}</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-200 text-left text-xs text-gray-500 uppercase tracking-wide">
                        <th className="pb-3 pr-4">Fatura No</th><th className="pb-3 pr-4">Tarih</th><th className="pb-3 pr-4">Hizmet Bedeli</th><th className="pb-3 pr-4">KDV</th><th className="pb-3 pr-4">Toplam</th><th className="pb-3">Durum</th>
                      </tr>
                    </thead>
                    <tbody>
                      {invoices.map((inv) => (
            ...
            {subscription ? (
              <div className="bg-gradient-to-r from-amber-50 via-orange-50 to-amber-50 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-lg font-black text-gray-900">{subscription.subscription_type} Gelişim Paketi Aktif</p>
              ...
              <div className="relative overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-6">
                <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-bl from-orange-600/20 to-transparent rounded-full blur-2xl" />
                <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-lg font-black text-white">Gelişim Paketine Geç</p>
                      <p className="text-sm text-gray-400">Öncelikli mentor eşleşmesi, sınırsız mesaj ve özel gelişim içerikleri.</p>
                    </div>
                  </div>
                  <Link to="/premium">
                    <Button className="rounded-xl bg-gradient-to-r from-orange-600 to-amber-500 text-white font-semibold hover:brightness-110 shadow-lg shadow-orange-900/30">
                      <Crown className="h-4 w-4 mr-2" /> Paketleri Gör
                    </Button>
                  </Link>
                </div>
              </div>
          ...
          <div className="rounded-2xl border border-orange-200 bg-white shadow-sm p-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-900">Doğru mentoru daha hızlı bul</p>
                  <p className="mt-1 text-sm text-gray-600">Gelişim hedefini netleştir → filtrele → kısa liste yap → ilk seansı planla.</p>
                </div>
              </div>
              <div className="flex gap-2 flex-wrap">
                <Link to="/how-it-works"><Button variant="outline" className="rounded-xl border-orange-200 hover:bg-orange-50 text-orange-600">Nasıl Çalışır?</Button></Link>
                <Link to="/coaches"><Button className="rounded-xl bg-gradient-to-r from-orange-600 to-amber-500 text-white hover:brightness-110">Mentorları İncele <ArrowRight className="h-4 w-4 ml-2" /></Button></Link>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* FOOTER LEGAL BANNER */}
        <div className="mt-10 p-5 bg-slate-100 rounded-2xl border border-slate-200 text-center">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-relaxed">
                Kariyeer.com bir Özel İstihdam Bürosu değildir. Bu dashboard üzerindeki tüm veriler "Eğitim ve Kariyer Mentorluğu" faaliyetlerinin takibi amacıyla sunulmaktadır.
            </p>
        </div>
      </div>
    </div>
