...
import { User, Pencil, X, Briefcase, CheckCircle2, MapPin, Camera, Target, History } from "lucide-react";

// --- GLOBAL AI DATA MODELLING (AB + UK + SAUDI + UAE) ---
const LOCATION_DATA = {
...
export default function UserProfile() {
  ...
  const handleFileUpload = async (e, type) => {
    ...
    try {
      ...
      if (uploadError) throw new Error("Bucket Ayarı Eksik: Supabase Storage panelinden 'profiles' bucket'ını 'Public' yapın.");
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-bold text-rose-500 animate-pulse uppercase tracking-widest text-lg">Sistem Senkronize Ediliyor...</div>;
    ...
    <div className="bg-[#F8FAFC] min-h-screen pb-12 font-sans overflow-x-hidden text-left">
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-5xl mx-auto">
          {/* BANNER AREA */}
          <div 
            className="h-32 md:h-44 bg-gradient-to-r from-[#e11d48] to-[#fb923c] relative cursor-pointer overflow-hidden rounded-b-xl group"
            onClick={() => coverInputRef.current?.click()}
          >
            {formData.cover_url && <img src={formData.cover_url} className="w-full h-full object-cover" />}
            <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center text-white font-black text-xs uppercase tracking-tighter"><Camera className="mr-2" size={16}/> Kapak Seç</div>
          </div>
          {/* PROFILE HEADER */}
          <div className="px-6 pb-6 flex flex-col md:flex-row items-end gap-6 -mt-12 relative z-10">
            ...
            <div className="flex-1 pb-2">
              <h1 className="text-xl md:text-2xl font-bold text-slate-800 uppercase tracking-tighter">{formData.full_name || "İSİM SOYİSİM"}</h1>
            ...
            <Button onClick={() => setEditOpen(true)} className="mb-2 bg-slate-50 text-slate-600 hover:bg-slate-100 rounded-xl font-bold border h-10 px-6 shadow-none uppercase text-[11px] tracking-widest">Edit Profile</Button>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
            <div className="p-6 border-b flex justify-between items-center bg-slate-50/50">
              <h2 className="text-lg font-bold text-slate-800 uppercase tracking-tight italic">Profil Mimarı</h2>
