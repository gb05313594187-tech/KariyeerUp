...
import { Calendar, Clock, User, Video, PlayCircle, CheckCircle2, Users, Radio } from 'lucide-react';
          ...
          <div className="flex justify-center gap-4">
            <Button
              ...
              className={`${activeTab === 'upcoming'
                ...
                : 'bg-red-800/50 text-white border border-white/20'
                } font-bold h-14 px-8 text-lg hover:bg-white hover:text-red-600 transition-all`}
            >
              Gelecek Etkinlikler
            </Button>
            <Button
        ...
        {activeTab === 'upcoming' && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {upcomingWebinars.map((webinar) => (
              <Card
                key={webinar.id}
                className="overflow-hidden hover:shadow-xl transition-all duration-300 border-t-4 border-t-red-500 group"
              >
                <div className="relative h-48 overflow-hidden">
                  ...
                  />
                  <Badge className="absolute top-4 right-4 bg-white/90 text-red-600 font-bold shadow-sm">
                    Canlı
                  </Badge>
                </div>
                <CardHeader>
                ...
                <CardContent>
                  <div className="flex justify-between items-center text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">
                    <div className="flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-red-500" /> {webinar.date}
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock className="w-4 h-4 text-red-500" /> {webinar.time}
                    </div>
                  </div>
        ...
        {activeTab === 'past' && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {pastWebinars.map((webinar) => (
              <Card
                key={webinar.id}
                className="overflow-hidden hover:shadow-lg transition-shadow opacity-90 group"
              >
                ...
                <CardContent>
                  <div className="flex justify-between text-sm text-gray-500 border-t pt-3">
                    ...
                    <span className="flex items-center gap-1">
                      <Users className="w-3 h-3" /> {webinar.views}
                    </span>
                  </div>
                </CardContent>
                <CardFooter>
